...
;(function($) {
	$.Loading = function(options) {
		...
		function pageRender(type) {
			var html = '';
			html += '<div class="loadingbox" id="loadingbox">';

			if (type === "html") {
				if (!!opts.random) {
					opts.loadPicNum = Math.ceil(Math.random() * 53);
				}
				html += '<div class="topPart"><img src="../js/registerLogin/loading/loading' + opts.loadPicNum + '.gif" /></div>'
			};
			if (type === "canvas") {
			...
			img.src = "../js/registerLogin/loading/loading" + opts.loadPicNum + ".gif";
		};
