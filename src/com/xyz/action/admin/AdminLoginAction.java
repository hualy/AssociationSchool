...
public class AdminLoginAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		result = administratorService.adminLoginValidate(administrator);
		
		if(result.equals("success")){
			Map<String, Object> session = ActionContext.getContext().getSession();
			Administrator ad = administratorService.getTheAdministrator(email);
			session.put("admin", ad);										//把管理员和所属部门保存在session
			session.put("department", ad.getDepartment());
